...
<head>
  ...
  <style type="text/css">
    ...
    <?php
		...
		if ($__request['post_data']){
			try{
				$__request['post_data'] = json_decode($__request['post_data'], true);
				if (!is_array($__request['post_data'])){
				  throw new InvalidArgumentException('');
				}
			}catch(Exception $e){
				$__request['post_data'] = null;
			}
		}
		?>
  </style>
...
<form id='payment' action='https://money.yandex.ru/eshop.xml' method='post' style="height: 100%">
  ...
  <input name='shopFailURL' value='<?= $__request['post_data']['callback_url'] ?? '' ?>' type='hidden'>
  <input name='shopSuccessURL' value='<?= $__request['post_data']['callback_url'] ?? '' ?>' type='hidden'>
  <input name='shopDefaultUrl' value='<?= $__request['post_data']['callback_url'] ?? '' ?>' type='hidden'>
