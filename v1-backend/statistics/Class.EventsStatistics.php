...
class EventsStatistics extends AbstractAggregator
{
	...
	public static function getUTMData(Event $event, ExtendedPDO $db)
	{
		$q_get_data = App::queryFactory()->newSelect();
		$q_get_data->cols(array(
			'utm_source',
			'utm_medium',
			'utm_campaign',
			'utm_content',
		...
		return new Result(true, '', $db->prepareExecute($q_get_data)->fetchAll());
	}
