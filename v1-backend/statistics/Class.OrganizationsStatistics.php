...
class OrganizationsStatistics extends AbstractAggregator
{
	...
	private function getValue($type, $scale, DateTime $since, DateTime $till)
	{
		...
		$q_get_data = $this->db->prepareExecuteRaw($query, $statements, 'CANT_INSERT_ORGANIZATION_VALUES');
	...
	private function getAudience()
	{
		...
		$p_get_devices = $this->db->prepareExecuteRaw(self::SQL_GET_AUDIENCE_DEVICES, $data, 'CANT_GET_DEVICES');
		$p_get_gender = $this->db->prepareExecuteRaw(self::SQL_GET_AUDIENCE_GENDER, $data, 'CANT_GET_GENDERS');
