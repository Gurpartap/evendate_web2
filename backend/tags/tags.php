...
$__modules['tags'] = array(
	'GET' => array(
		'all' => function () use ($__db, $__user, $__length, $__page) {
			return TagsCollection::all($__db, ' ORDER BY organizations.name ASC LIMIT ' . ($__page * $__length) . ' , ' . $__length, $__user);
		},
		...
		'search' => function () use ($__request, $__db, $__user, $__length, $__page) {
			...
			return TagsCollection::search($__request['q'], $__db, ' ORDER BY organizations.name ASC LIMIT ' . ($__page * $__length) . ' , ' . $__length, $__user);
		}
	)
);