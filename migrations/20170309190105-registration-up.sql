ALTER TABLE log_requests
  ADD COLUMN db_query TEXT DEFAULT NULL;


CREATE VIEW view_events_tickets_sold AS
  SELECT
    COUNT(tickets.id),
  ...
  SELECT DISTINCT
    ...
    (events.registration_locally = TRUE
     ...
     AND (events.registration_limit_count >
          COALESCE((SELECT view_events_tickets_sold.count FROM view_events_tickets_sold WHERE
                   view_events_tickets_sold.event_id = events.id), 0)::INT OR events.registration_limit_count IS NULL)
