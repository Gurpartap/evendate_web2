ALTER TABLE events
  ADD COLUMN first_event_date TIMESTAMPTZ;

ALTER TABLE events
  ADD COLUMN last_event_date TIMESTAMPTZ;

CREATE INDEX ON tokens(user_id);

UPDATE events
SET first_event_date = (SELECT MIN((events_dates.event_date :: DATE || ' ' || events_dates.start_time) :: TIMESTAMPTZ)
                        FROM events_dates
                        WHERE event_id = events.id AND events_dates.status = TRUE);


UPDATE events
SET last_event_date = (SELECT MAX((events_dates.event_date :: DATE || ' ' || events_dates.start_time) :: TIMESTAMPTZ)
                       FROM events_dates
                       WHERE event_id = events.id AND events_dates.status = TRUE);
